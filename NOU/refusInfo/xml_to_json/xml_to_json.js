...
const jsonFilePath = path.join(__dirname, 'refusInfo.json');
  ...
  for (const nodeXml of nodes) {
    try {
      ...
      if (idMatch) {
        ...
        nodeData.id = parseInt(idValue, 10);
      }
      ...
      if (typeIdMatch) {
        const typeIdValue = typeIdMatch[1].trim();
        nodeData.type.id = parseInt(typeIdValue, 10);
      }
      ...
      if (placesValeurMatch) {
        const placesValue = placesValeurMatch[1].trim();
        // Convert to integer if it's a valid number
        nodeData.places.valeur = /^\d+$/.test(placesValue) ? parseInt(placesValue, 10) : placesValue;
      }
      ...
      if (etatIdMatch) {
        const etatIdValue = etatIdMatch[1].trim();
        nodeData.etat.id = parseInt(etatIdValue, 10);
      }
      ...
      // Extract derniere_modif
      const derniereModifMatch = nodeXml.match(/<derniere_modif>([^<]+)<\/derniere_modif>/);
      if (derniereModifMatch) nodeData.derniere_modif = derniereModifMatch[1].trim();
